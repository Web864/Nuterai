/**
 * Reusable food-capture panels — photo scan, barcode scan, and text search.
...
export function PhotoTab({ userId }: { userId: string }) {
  ...
  async function runAnalyze() {
    ...
    try {
      ...
      toast.error(describeAnalysisError(e));
    } finally {
      setBusy(false);
    }
  }
          ...
          <div className="space-y-4">
            ...
            {result && items.length === 0 && (
              <p className="text-sm text-muted-foreground text-center">
                {result.notes ?? "We couldn't identify the food clearly. Please try another photo."}
              </p>
            )}
          </div>
        )}
...
export function BarcodeTab({ userId }: { userId: string }) {
  const qc = useQueryClient();
  ...
  async function handleBarcode(code: string) {
    ...
    setProduct(null);
    try {
      const p = await lookup({ data: { barcode: trimmed } });
      if (!p) {
        setNotFound(trimmed);
        toast.error("We couldn't find this product. You can enter the meal manually.");
      } else {
        setProduct(p);
      }
    } catch (e) {
  ...
  async function startScan() {
    ...
    try {
      ...
      if (e instanceof DOMException && e.name === "NotAllowedError") {
        toast.error("Camera permission was denied. Please allow camera access.");
      } else {
        toast.error("Couldn't access the camera. Please try manual entry.");
      }
      setScanning(false);
    }
  }
  ...
  function submitManual(e: React.FormEvent) {
    e.preventDefault();
    void handleBarcode(manual);
  }

  const analyzing = busy && !scanning && !product && !notFound;
...
export function SearchTab({ userId }: { userId: string }) {
  const qc = useQueryClient();
